...
|====
+
3. Enable IP forward settings on all the worker node(s) so that, the containers are able to ping the internet using the following commands
+
....
...
....
7. Copy the contents of SSH public key of the Master node in the `authorized_keys`
...
....
8. It is recommended to login as root on all the nodes, as we will be dealing with files and tools which require
