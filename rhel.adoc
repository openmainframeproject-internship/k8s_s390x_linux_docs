= How to  deploy a Kubernetes cluster on RHEL 7.4 flavour of LinuxONE instances?
...
....
scp -i master.pem worker1.pem worker2.pem root@148.100.98.174:/root
....
...
NOTE: In this documnetation whereever 148.100.98.173 is used you should use your master ip there and inplace of 148.100.98.174 use your worker ip.
== Certificates
...
....
scp -i <path_where_your_worker_keys_are> ca.pem etcd.crt etcd.key kubelet.pem kubelet-key.pem root@148.100.98.174:/srv/kubernetes/
....
...
scp k8sworker-worker.kubeconfig root@148.100.98.174:/srv/kubernetes/kubelet.kubeconfig
....
...
Modify the configuration file in the /etc/etcd/etcd.conf should look like this after all the modifications. But do note to change the ipaddresses
